...
import type { OrderOperation } from '../orders/order-store-types.js';
...
export const orders = pgTable('orders', {
  ...
  uniqueIndex('orders_creation_request_unique').on(table.creationRequestId),
...
// Client commands are distinct from individual transition IDs and provider keys.
export const orderCommands = pgTable('order_commands', {
  id: uuid('id').defaultRandom().primaryKey(),
  orderId: uuid('order_id').notNull().references(() => orders.id, { onDelete: 'restrict' }),
  requestId: text('request_id').notNull(),
  operation: text('operation').$type<OrderOperation>().notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
  uniqueIndex('order_commands_request_unique').on(table.orderId, table.requestId),
  check('order_commands_request_nonempty', sql`${table.requestId} ~ '[^[:space:]]'`),
  check('order_commands_operation_valid', sql`${table.operation} in ('authorizePayment', 'completeOrder', 'cancelOrder')`),
]);
