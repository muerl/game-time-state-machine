CREATE TABLE "order_commands" (
	...
	CONSTRAINT "order_commands_request_nonempty" CHECK ("order_commands"."request_id" ~ '[^[:space:]]'),
	CONSTRAINT "order_commands_operation_valid" CHECK ("order_commands"."operation" in ('authorizePayment', 'completeOrder', 'cancelOrder'))
);
...
CREATE TABLE "order_transitions" (
	...
	CONSTRAINT "order_transitions_event_valid" CHECK ("order_transitions"."event" in ('order_initialized', 'payment_authorization_started', 'payment_authorized', 'payment_declined', 'completion_started', 'payment_void_started', 'order_completed', 'payment_voided', 'payment_void_failed', 'order_cancelled', 'payment_authorization_unconfirmed', 'completion_unconfirmed')),
...
--> statement-breakpoint
ALTER TABLE "order_commands" ADD CONSTRAINT "order_commands_order_id_orders_id_fk" FOREIGN KEY ("order_id") REFERENCES "public"."orders"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "order_commands_request_unique" ON "order_commands" USING btree ("order_id","request_id");--> statement-breakpoint
...
CREATE UNIQUE INDEX "orders_creation_request_unique" ON "orders" USING btree ("creation_request_id");--> statement-breakpoint
