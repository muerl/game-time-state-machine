...
`complete`, `rejected`, `cancelled`, and `needs_attention` have no outgoing transitions in this prototype. The service enforces this graph and records every transition. Unconfirmed authorization/completion results record a self-transition in the pending state with a sanitized failure and an incremented version.
